...
- **Closures / higher-order functions:** lambdas, captures, currying, and applied function values now **compile and run in the native (C) backend** — lambda-lifted to top-level functions, captured into reference-counted closure cells, dispatched through a function-pointer table — and are verified differentially against the interpreter oracle **garbage-free** (`aria_live == 0`): immediate application, currying, scalar AND heap captures, generic higher-order `map` with capture + function-by-name, applied-twice, and `compose` (closures capturing closures). The **wasm** backend still rejects closures with a clean error (`call_indirect` + function-table support is the remaining item). One narrow front-end gap: an *unannotated* lambda bound to a bare `let` with no contextual type (`let f = \x -> ..`) gives a clean "annotate the parameter" error — typeck infers the type but does not yet write it back into the AST.
